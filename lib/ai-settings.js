...
const PROVIDERS = new Set(["deepseek", "newapi", "custom"]);
...
function inferProvider(value) {
  try {
    return new URL(String(value || DEFAULT_BASE_URL)).hostname === "api.deepseek.com" ? "deepseek" : "custom";
  } catch {
    return "deepseek";
  }
}
...
function normalizeProvider(value, baseUrl) {
  ...
  return PROVIDERS.has(provider) ? provider : inferProvider(baseUrl);
}
...
function normalizeBaseUrl(value, provider) {
  ...
  if (/\/(?:chat\/completions|models)\/?$/i.test(url.pathname)) {
    throw new Error("API Base URL 不要包含 /chat/completions 或 /models");
  }
  if (provider === "newapi" && !/\/v1\/?$/i.test(url.pathname)) {
...
export function normalizeAiSettings(input = {}, current = {}) {
  const provider = normalizeProvider(input.provider || current.provider, input.baseUrl || current.baseUrl);
  const baseUrl = normalizeBaseUrl(input.baseUrl || current.baseUrl || DEFAULT_BASE_URL, provider);
  const submittedKey = String(input.apiKey || "").trim();
  const sameEndpoint = !current.baseUrl || (
    provider === normalizeProvider(current.provider, current.baseUrl) &&
    baseUrl === normalizeBaseUrl(current.baseUrl, normalizeProvider(current.provider, current.baseUrl))
  );
  const apiKey = submittedKey || (sameEndpoint ? String(current.apiKey || "").trim() : "");
...
export function aiSettingsEnvironment(settings, baseEnvironment = process.env) {
  return {
    ...
    AI_THINKING: settings.thinking,
    AI_MAX_TOKENS: String(settings.maxTokens),
    AI_TIMEOUT_MS: String(settings.timeoutMs),
    AI_MOCK: "0",
