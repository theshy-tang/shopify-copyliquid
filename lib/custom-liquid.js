...
export function getDeepSeekConfig(env = process.env) {
  const requestedModel = String(env.DEEPSEEK_MODEL || DEFAULT_MODEL).trim();
  const model = /^[a-zA-Z0-9._:/-]{1,160}$/.test(requestedModel) ? requestedModel : DEFAULT_MODEL;
  const baseUrl = String(env.DEEPSEEK_BASE_URL || DEFAULT_BASE_URL).trim().replace(/\/+$/, "");
  const mock = String(env.DEEPSEEK_MOCK || "").trim() === "1";
  return {
    apiKey: String(env.DEEPSEEK_API_KEY || "").trim(),
    baseUrl,
    model,
    mock,
    configured: mock || Boolean(String(env.DEEPSEEK_API_KEY || "").trim()),
    thinking: String(env.DEEPSEEK_THINKING || "disabled").trim() === "enabled" ? "enabled" : "disabled",
    maxTokens: toBoundedInteger(env.DEEPSEEK_MAX_TOKENS, DEFAULT_MAX_TOKENS, 4096, 192000),
    timeoutMs: toBoundedInteger(env.DEEPSEEK_TIMEOUT_MS, DEFAULT_TIMEOUT_MS, 30000, 600000)
  };
}
...
export function buildConversionMessages({ module, html, css, sourceUrl, namespace, reviewInventory = null }) {
...
</MATCHED_SOURCE_CSS>${reviewRequirementsMarkup(reviewInventory)}`;
...
export function validateCustomLiquid(code, namespace, { allowOversize = false } = {}) {
  ...
  if (!normalized) {
    throw new ConversionError("DeepSeek 没有返回 Custom Liquid 代码，请重试", {
  ...
  if (Buffer.byteLength(normalized, "utf8") > MAX_OUTPUT_BYTES) {
    throw new ConversionError("DeepSeek 返回的代码超过安全大小限制", {
  ...
  if (/<!doctype|<\/?(?:html|head|body)\b/i.test(normalized) || /{%\s*schema\s*%}/i.test(normalized)) {
    throw new ConversionError("DeepSeek 返回了完整页面或 Section 文件，不是 Custom Liquid 代码", {
  ...
  if (!new RegExp(`id=["']${escapedNamespace}["']`, "i").test(normalized)) {
    throw new ConversionError("DeepSeek 返回的代码缺少独立根节点，无法安全隔离样式", {
...
export function parseDeepSeekResult(content, namespace, { allowOversize = false } = {}) {
  ...
  try {
    ...
    throw new ConversionError("DeepSeek 返回的数据格式无法解析，请重试", {
...
function providerError(status, detail = "") {
  const errors = {
    400: ["DeepSeek 无法识别请求内容", "DEEPSEEK_INVALID_FORMAT", false],
    401: ["DeepSeek API Key 无效，请检查 .env 配置", "DEEPSEEK_AUTH_FAILED", false],
    402: ["DeepSeek 账户余额不足，请充值后重试", "DEEPSEEK_BALANCE_EMPTY", false],
    422: ["DeepSeek 模型参数无效，请检查模型配置", "DEEPSEEK_INVALID_PARAMETERS", false],
    429: ["DeepSeek 请求过于频繁，请稍后重试", "DEEPSEEK_RATE_LIMITED", true],
    500: ["DeepSeek 服务暂时出错，请重试", "DEEPSEEK_SERVER_ERROR", true],
    503: ["DeepSeek 当前负载较高，请稍后重试", "DEEPSEEK_OVERLOADED", true]
  };
  const [message, code, retryable] = errors[status] || ["无法连接 DeepSeek，请检查网络后重试", "DEEPSEEK_REQUEST_FAILED", status >= 500];
...
async function requestDeepSeekCompletion({ config, messages, fetchImpl, signal }) {
  ...
  try {
    response = await fetchImpl(`${config.baseUrl}/chat/completions`, {
      ...
      body: JSON.stringify({
        model: config.model,
        messages,
        ...(config.thinking === "enabled" ? { thinking: { type: "enabled" }, reasoning_effort: "high" } : {}),
    ...
    if (error?.name === "AbortError") {
      ...
      throw new ConversionError("DeepSeek 生成超时，请重试或缩小模块", {
  ...
  try {
    ...
    if (!response.ok) throw providerError(response.status, String(responseText || "").slice(0, 1000));
    throw new ConversionError("DeepSeek 返回了无法解析的响应，请重试", {
      code: "INVALID_DEEPSEEK_RESPONSE",
      status: 502,
      retryable: true
    });
  }
  if (!response.ok) throw providerError(response.status, JSON.stringify(payload?.error || payload).slice(0, 1000));

  const choice = payload?.choices?.[0];
  if (!choice?.message?.content) {
    throw new ConversionError("DeepSeek 没有返回生成结果，请重试", {
  ...
  if (choice.finish_reason === "length") {
    throw new ConversionError("生成结果达到长度上限，请提高 DEEPSEEK_MAX_TOKENS 或缩小模块", {
...
export async function convertModuleToCustomLiquid({ module, css, sourceUrl, replacements, reviewLimit, namespace, env = process.env, fetchImpl = fetch, signal }) {
  ...
  if (!config.configured) {
    throw new ConversionError("尚未配置 DeepSeek API Key，请先在 .env 中设置 DEEPSEEK_API_KEY", {
  ...
  return {
    ...
    usage: mergeUsage(firstCompletion.usage, repairCompletion?.usage),
    model: repairCompletion?.model || firstCompletion.model,
    requestAudit: {
      sourceBytes,
      selectedSourceBytes,
