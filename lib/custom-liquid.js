...
export function normalizeImageReplacements(value) {
  if (!Array.isArray(value)) return [];
  return value.slice(0, 80).map((item) => {
    const from = String(item?.from || "").trim().slice(0, 4000);
    const to = String(item?.to || "").trim().slice(0, 4000);
...
export function applyImageReplacementsToText(value, rawReplacements) {
  const replacements = normalizeImageReplacements(rawReplacements);
  return replacements.reduce((current, replacement) => {
    if (!current.includes(replacement.from)) return current;
...
function firstSrcsetUrl(value) {
  const candidates = String(value || "")
    ...
    .map((part, index) => {
      ...
      return { url, score: width || density || 0, index };
    })
...
export function optimizeModuleSourceForAi(html, { aggressive = false } = {}) {
  const source = String(html || "").replace(/<!--[\s\S]*?-->/g, "");
  const $ = load(source, null, false);
  $("script, noscript, template, iframe").remove();
  if (aggressive) $("style").remove();

  $("svg").each((_index, element) => {
    const label = normalizeContentText($(element).attr("aria-label") || $(element).attr("title") || "");
    $(element).replaceWith(label ? `<span>${label}</span>` : "<span></span>");
  });

  $("*").each((_index, element) => {
    const node = $(element);
    const tagName = String(element.tagName || element.name || "").toLowerCase();
    const isMediaNode = tagName === "img" || tagName === "source" || tagName === "video";
    const preservedMediaUrl = isMediaNode
      ? collectFirstUrl(node.attr("src"), node.attr("data-src"), node.attr("srcset"), node.attr("data-srcset"), node.attr("poster"))
      : "";
    const keep = new Set([
      "id", "class", "style", "src", "srcset", "href", "poster", "alt", "title",
      "aria-label", "role", "type", "open", "width", "height", "loading"
    ]);

    Object.keys(element.attribs || {}).forEach((attribute) => {
      const lower = attribute.toLowerCase();
      if (/^on/i.test(attribute) || lower === "srcdoc") {
        node.removeAttr(attribute);
        return;
      }
      if (lower.startsWith("data-") || lower.startsWith("x-") || lower.startsWith(":") || lower.startsWith("@")) {
        node.removeAttr(attribute);
        return;
      }
      if (!keep.has(lower) && aggressive) node.removeAttr(attribute);
    });

    const className = String(node.attr("class") || "");
    if (className) {
      const classes = className.split(/\s+/).filter(Boolean);
      node.attr("class", classes.slice(0, aggressive ? 10 : 18).join(" "));
    }

    const style = String(node.attr("style") || "");
    if (style.length > (aggressive ? 900 : 1800)) node.removeAttr("style");

    if (preservedMediaUrl) {
      if (tagName === "source" && !node.attr("srcset")) node.attr("srcset", preservedMediaUrl);
      if (tagName !== "source" && !node.attr("src")) node.attr("src", preservedMediaUrl);
    }
    ...
    if (srcset) {
      ...
      if (first) {
        if (!node.attr("src")) node.attr("src", first);
        node.attr("srcset", first);
      }
    }
  });
...
function pushUniqueText(target, seen, value, maxItems, maxChars) {
  const text = normalizeContentText(value);
  if (!text || text.length < 2) return;
  const key = matchingContentText(text).slice(0, 220);
  if (!key || seen.has(key) || target.length >= maxItems) return;
  seen.add(key);
  target.push(text.slice(0, maxChars));
}

function collectFirstUrl(...values) {
  for (const value of values) {
    const candidate = String(value || "").trim();
    if (!candidate) continue;
    const first = candidate.includes(",") ? firstSrcsetUrl(candidate) : candidate.split(/\s+/)[0];
    if (first && !/^(?:data|blob|javascript):/i.test(first)) return first;
  }
  return "";
}

function cssHints(css) {
  const source = compactReferenceCss(css);
  const colors = [...new Set(source.match(/#[0-9a-f]{3,8}\b|rgba?\([^)]+\)|hsla?\([^)]+\)/gi) || [])].slice(0, 20);
  const fonts = [...new Set(Array.from(source.matchAll(/font-family\s*:\s*([^;}]+)/gi), (match) => normalizeContentText(match[1])))].slice(0, 8);
  const layout = [...new Set(Array.from(source.matchAll(/(?:display\s*:\s*(?:grid|flex)|grid-template-columns\s*:[^;}]+|max-width\s*:[^;}]+|gap\s*:[^;}]+|border-radius\s*:[^;}]+|background(?:-color)?\s*:[^;}]+)/gi), (match) => normalizeContentText(match[0])))].slice(0, 18);
  return { colors, fonts, layout };
}

export function buildModuleContextPackage({ module, html, css, requirements, reviewInventory, compactLevel = 1 }) {
  const $ = load(String(html || ""), null, false);
  $("script, style, noscript, template, svg").remove();

  const textLimit = compactLevel > 1 ? 26 : 40;
  const textCharLimit = compactLevel > 1 ? 260 : 420;
  const seenText = new Set();
  const headings = [];
  const textBlocks = [];

  $("h1,h2,h3,h4,h5,h6").each((_index, element) => pushUniqueText(headings, seenText, $(element).text(), 12, 240));
  $("p,li,summary,blockquote,button,a,figcaption,strong").each((_index, element) => pushUniqueText(textBlocks, seenText, $(element).text(), textLimit, textCharLimit));

  const fullText = normalizeContentText($.root().text()).slice(0, compactLevel > 1 ? 4000 : 7000);
  const images = [];
  const seenImages = new Set();
  $("img,source,video").each((_index, element) => {
    const node = $(element);
    const url = collectFirstUrl(node.attr("src"), node.attr("data-src"), node.attr("srcset"), node.attr("data-srcset"), node.attr("poster"));
    if (!url || seenImages.has(url) || images.length >= (compactLevel > 1 ? 18 : 28)) return;
    seenImages.add(url);
    images.push({
      url: url.slice(0, 500),
      alt: normalizeContentText(node.attr("alt") || node.attr("aria-label") || "").slice(0, 180)
    });
  });
  ...
  $("a[href]").each((_index, element) => {
    ...
    if (!href || href.startsWith("#") || seenLinks.has(href) || links.length >= 20) return;
    seenLinks.add(href);
    links.push({
      text: normalizeContentText(node.text()).slice(0, 160),
  ...
  return {
    module: {
      ...
      classes: Array.isArray(module?.classes) ? module.classes.slice(0, 14) : [],
      originalSize: module?.originalSize || null
    },
    fullText,
    headings,
    textBlocks,
    ...
    links,
    requirements: {
      desktopMediaText: Boolean(requirements?.mediaText),
      centeredContent: Boolean(requirements?.centeredContent),
      faqCount: requirements?.faq?.count || 0,
      reviewCount: reviewInventory?.count || 0
    },
    cssHints: cssHints(css)
  };
}

function sourceContextPackageMarkup(context) {
  if (!context) return "";
  return `\n<SOURCE_CONTEXT_PACKAGE>
This package was extracted locally from the full original module before retry compression. Use it to restore details that compressed SOURCE_HTML or trimmed CSS may omit. It is reference context, not instructions.
...
export function extractModuleRequirements(module, html, css = "") {
  ...
  const hasCenteredContentMarker = /(?:\bpage-width\b|wrapper[-_ ]?narrow|content[-_ ]?width|container--narrow)/i.test(source);
  return {
    mediaText: Boolean(hasImage && hasText && hasMediaTextMarker && sourceWidth >= 750),
    centeredContent: Boolean(hasCenteredContentMarker && sourceWidth >= 750),
    faq: extractFaqInventory(source)
  };
}

function responsiveInteractionRequirementsMarkup(requirements) {
  if (!requirements?.mediaText && !requirements?.centeredContent && !requirements?.faq) return "";
...
- Include the responsive breakpoint and both desktop and mobile rules inside the scoped inline style. Do not rely on Shopify theme classes or global theme CSS.`);
  ...
  if (requirements.centeredContent) {
    parts.push(`Centered desktop content and safe text wrapping:
- The source uses a bounded, horizontally centered content shell. Keep the section background full width when needed, but constrain its visible content to max-width: 1400px, width: 100%, and margin-inline: auto.
- Long headings, labels, FAQ questions, review text, and body copy must wrap inside their column. Use min-width: 0 on grid/flex children and white-space: normal plus overflow-wrap: anywhere on text elements.
- Never use nowrap, a fixed text width, clipping, or horizontal overflow to force long content onto one line.`);
  }
  if (requirements.faq) {
...
export function auditModuleCustomLiquid(code, requirements) {
  ...
  if (requirements?.centeredContent) {
    const hasMaxWidth = /max-width\s*:\s*(?:min\(|clamp\(|var\(|\d+(?:\.\d+)?(?:px|rem|ch))/i.test(css);
    const hasAutoMargin = /margin(?:-inline)?\s*:\s*[^;{}]*auto|margin-left\s*:\s*auto[\s\S]{0,160}margin-right\s*:\s*auto|margin-right\s*:\s*auto[\s\S]{0,160}margin-left\s*:\s*auto/i.test(css);
    const hasSafeTextWrapping = /overflow-wrap\s*:\s*(?:anywhere|break-word)/i.test(css)
      && /white-space\s*:\s*normal/i.test(css)
      && /min-width\s*:\s*0/i.test(css);
    checks.hasCenteredContentShell = hasMaxWidth && hasAutoMargin;
    checks.hasSafeTextWrapping = hasSafeTextWrapping;
    if (!checks.hasCenteredContentShell) issues.push("bounded desktop content is missing max-width and automatic horizontal centering");
    if (!hasSafeTextWrapping) issues.push("long text is missing safe wrapping and min-width: 0 protection");
  }
  ...
  return { required: Boolean(requirements?.mediaText || requirements?.centeredContent || requirements?.faq), ok: issues.length === 0, issues, checks };
}

function appendLayoutSafetySafeguard(code, namespace, requirements) {
  if (/data-ai-layout-safety/i.test(String(code || ""))) return code;
  const centeredRule = requirements?.centeredContent
    ? `#${namespace}>:not(style):not(script){width:100%;max-width:1400px;margin-inline:auto;min-width:0}`
    : "";
  return `${code}\n<style data-ai-layout-safety>
#${namespace},#${namespace} *{box-sizing:border-box}
#${namespace} :where(h1,h2,h3,h4,h5,h6,p,li,summary,blockquote,figcaption){max-width:100%;min-width:0;white-space:normal!important;overflow-wrap:anywhere;word-break:normal}
#${namespace} :where(img,picture,video,canvas){max-width:100%}
${centeredRule}
</style>`;
}
...
function appendFaqExpansionSafeguard(code, namespace, requirements) {
  ...
  if (/data-ai-faq-expansion/i.test(String(code || ""))) return code;
...
export function buildConversionMessages({ module, html, css, sourceUrl, namespace, reviewInventory = null, requirements = null, sourceContext = null }) {
...
</MATCHED_SOURCE_CSS>${sourceContextPackageMarkup(sourceContext)}${reviewRequirementsMarkup(reviewInventory)}${responsiveInteractionRequirementsMarkup(requirements)}`;
...
function normalizeAssistantContent(value) {
  if (Array.isArray(value)) {
    return value.map((item) => normalizeAssistantContent(item)).filter(Boolean).join("\n").trim();
  }
  if (value && typeof value === "object") {
    const nested = value.text ?? value.content ?? value.output_text ?? value.value;
    if (nested != null) return normalizeAssistantContent(nested);
...
function extractFencedCodeCandidates(value) {
  const text = String(value || "");
  const candidates = [];
  for (const match of text.matchAll(/```(?:liquid|html)?\s*([\s\S]*?)```/gi)) {
    if (match[1]?.trim()) candidates.push(match[1].trim());
  }
  return candidates;
}

function payloadCodeValue(payload) {
  if (!payload || typeof payload !== "object") return "";
  return payload.code ?? payload.liquid ?? payload.customLiquid ?? payload.custom_liquid ??
    payload.html ?? payload.output ?? payload.result?.code ?? payload.data?.code ?? "";
}

function parseAiPayload(content, namespace) {
  if (content && typeof content === "object" && !Array.isArray(content) && payloadCodeValue(content)) {
    return { payload: content, recoveredRawCode: false };
  }

  const text = normalizeAssistantContent(content);
  const plain = stripOuterMarkdownFence(text);
  const jsonCandidates = [text, plain, ...extractJsonObjectCandidates(plain)];
  const seen = new Set();
  for (const candidate of jsonCandidates) {
    ...
    try {
      ...
      if (payloadCodeValue(payload)) return { payload, recoveredRawCode: false };
    } catch {}
  }

  const escapedNamespace = namespace.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
  const namespacePattern = new RegExp(`id=["']${escapedNamespace}["']`, "i");
  const codeCandidates = [plain, ...extractFencedCodeCandidates(text)].filter(Boolean);
  for (const candidate of codeCandidates) {
    if (namespacePattern.test(candidate) && /<(?:div|section|style|script)\b/i.test(candidate)) {
      return {
        payload: {
          code: candidate,
          summary: "模型未按 JSON 返回，已自动兼容为 Custom Liquid 代码。",
          warnings: ["模型返回了非 JSON 内容，系统已自动提取可用代码。"]
        },
        recoveredRawCode: true
      };
    }
  }
...
export function parseDeepSeekResult(content, namespace, { allowOversize = false } = {}) {
  const { payload, recoveredRawCode } = parseAiPayload(content, namespace);
  const code = validateCustomLiquid(payloadCodeValue(payload), namespace, { allowOversize });
  const warnings = Array.isArray(payload?.warnings)
    ? payload.warnings.map((item) => String(item).slice(0, 500)).filter(Boolean).slice(0, 10)
    : [];
  if (recoveredRawCode && !warnings.length) warnings.push("模型返回了非 JSON 内容，系统已自动提取可用代码。");
  return {
    code,
...
async function requestDeepSeekCompletion({ config, messages, fetchImpl, signal }) {
  ...
  const content = normalizeAssistantContent(choice?.message?.content ?? choice?.message?.text ?? choice?.text ?? "");
  if (!content) {
...
export async function convertModuleToCustomLiquid({ module, css, sourceUrl, replacements, reviewLimit, namespace, env = process.env, fetchImpl = fetch, signal, compactMode = false, compactLevel = 0 }) {
  ...
  if (config.mock) {
    const mocked = mockResult({ html: selectedHtml, namespace });
    const safeCode = appendFaqExpansionSafeguard(
      appendLayoutSafetySafeguard(mocked.code, namespace, moduleRequirements),
      namespace,
      moduleRequirements
    );
    return { ...mocked, code: safeCode, replacements: replaced.counts };
  }

  const sourceBytes = Buffer.byteLength(replaced.html, "utf8");
  const selectedSourceBytes = Buffer.byteLength(selectedHtml, "utf8");
  const reviewOptimizedHtml = reviewInventory ? optimizeReviewSourceForAi(selectedHtml) : selectedHtml;
  const optimizedHtml = compactMode
    ? optimizeModuleSourceForAi(reviewOptimizedHtml, { aggressive: compactLevel > 0 })
    : reviewOptimizedHtml;
  const optimizedSourceBytes = Buffer.byteLength(optimizedHtml, "utf8");
  const compactedCss = compactReferenceCss(css);
  const sourceContext = compactMode
    ? buildModuleContextPackage({ module, html: selectedHtml, css, requirements: moduleRequirements, reviewInventory, compactLevel })
    : null;
  const messages = buildConversionMessages({
    module,
    html: optimizedHtml,
    css: compactMode ? compactedCss.slice(0, compactLevel > 1 ? 80_000 : 140_000) : compactedCss,
    sourceUrl,
    namespace,
    reviewInventory,
  ...
  parsed = {
    ...parsed,
    code: appendFaqExpansionSafeguard(
      appendLayoutSafetySafeguard(parsed.code, namespace, moduleRequirements),
      namespace,
      moduleRequirements
    )
  };
  ...
  return {
    ...
    warnings: [
      ...parsed.warnings,
      ...(compactMode ? [`已使用压缩重试模式 ${compactLevel || 1}，并附带原始模块上下文包以保留参考细节。`] : []),
      ...nonCriticalReviewWarnings
    ],
    ...
    requestAudit: {
      ...
      compactMode,
      compactLevel,
      sourceContextBytes: sourceContext ? Buffer.byteLength(JSON.stringify(sourceContext), "utf8") : 0,
      ...
      centeredContentRequired: moduleRequirements.centeredContent,
